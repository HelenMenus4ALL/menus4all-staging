...
async function pushMenuToProduction(menuId) {
    try {
        ...
        const today = new Date().toISOString().split('T')[0]; // Format: "2025-11-30"
        ...
        const productionData = {
            restaurantInfo: {
                ...
                stateCode: stagingMenu.state || '',  // Add stateCode (same as state)
                slug: restaurantSlug,
                ...
                requesterName: stagingMenu.requesterName || '',  // Add requesterName field
                hours: stagingMenu.hours || [],
                lat: stagingMenu.latitude ? parseFloat(stagingMenu.latitude) : null,  // null if not provided
                lng: stagingMenu.longitude ? parseFloat(stagingMenu.longitude) : null,   // null if not provided
                lastUpdated: today  // Add lastUpdated as date string
            },
            menu: stagingMenu.menuJson || {}
        };
        
        // DON'T remove null values - production expects null for missing lat/lng
